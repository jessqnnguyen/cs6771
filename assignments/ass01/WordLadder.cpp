...
vector<vector<std::string>> getAllOptimalWordLadders(
		const std::string &startWord, const std::string &destWord);
...
void pushChildWordsToQueue(const vector<std::string> &wordLadder,
		const std::string &destWord, Lexicon &english, Lexicon &visitedWords,
...
vector<std::string> getAllValidChildWords(const std::string &word,
		const std::string &destWord, Lexicon &english,
...
vector<vector<std::string>> getAllOptimalWordLadders(
	...
	visitedWordsLength.insert(
			std::pair<std::string, unsigned int>(startWord,
					startWordLadder.size()));

	// Create a queue for BFS.
	queue<Node> q;
	Node n;
	pushChildWordsToQueue(startWordLadder, destWord, english, visitedWords,
	...
	optimalWordLadders.erase(
			std::unique(optimalWordLadders.begin(), optimalWordLadders.end()),
...
void pushChildWordsToQueue(const vector<std::string> &wordLadder,
	...
	vector<std::string> validChildWords = getAllValidChildWords(fromWord,
			destWord, english, lastIndexModified);
	for (const auto childWord : validChildWords) {
		...
		if (wordHasNotBeenVisited) {
			...
			visitedWordsLength.insert(
					std::pair<std::string, unsigned int>(childWord,
							currWordLadder.size()));
		} else {
			...
			if (wordWasVisitedAtALongerLadder) {
				...
				// visitedWordsLength[childWord] = currWordLadder.size();
			}
		}
